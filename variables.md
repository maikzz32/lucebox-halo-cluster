...
| `DFLASH_QWEN4EXP_FUSE_HC` | ⚡ **performance.** Collapse the hyper-connection mixer's gate-and-average into one kernel instead of nine (a multiply, four contiguous copies, three adds, a scale), ninety-six times a token. 25.0 → 26.5 tok/s on one node, 27.9 → 30.5 on two. Verified against the unfused path to 2.4e-07 by `hc_collapse_check`. Off by default; the unfused path is the reference and serves the CPU tools. |
...
| `DFLASH_QWEN4EXP_NO_SHARD_EXPERTS` | Keep the routed experts whole on a cluster, removing 48 of the 97 reductions at the cost of their bytes. Measured level with sharding them — expert parallelism itself breaks even on this fabric. |
| `DFLASH_CLUSTER_SEGMENTED_GRAPH` | Capture a cluster forward in segments cut at every collective, so the kernels replay and only the collective is launched by hand. Isolates the reductions: 29.6 ms of GPU work against a 41 ms step on two nodes. Neutral on throughput today, because the reductions cost the same however they are issued. |
| `DFLASH_CLUSTER_FAST_REDUCE` | Lean decode-path reduction: each rank writes its partial straight into the peers' pre-registered buffers instead of calling a collective, with the flags raised and waited on by the command processor rather than a kernel. Correct and **level with RCCL** (27.5 against 27.3 tok/s on two nodes with speculation) — what a reduction costs here is the synchronisation point, not the protocol. Decode sizes only; prefill falls back. Needs `--ulimit memlock=-1` on the container. |
| `DFLASH_CLUSTER_FAST_REDUCE_ELEMS` / `_SLOTS` | Size of the lean fabric: floats per reduction (default 16384, a decode step's largest is 10240) and in-flight ring slots (default 4096). The ring has to be deep: its guard is a stream wait, and one that fires stops the GPU running ahead, which is where the speed comes from. Anything larger than `_ELEMS` falls back to RCCL. |
| `DFLASH_CLUSTER_FAST_REDUCE_KERNEL_FLAGS` | Raise and wait on the flags with kernels instead of command-processor stream operations. Measured **250x slower**: a system-scope release writes the L2 back and an acquire invalidates it, and the weights go with it. Kept because it is the obvious way to write this. |
| `DFLASH_CLUSTER_FAST_REDUCE_DRYRUN` | Cost probe. Keeps every local operation of the lean path and drops the waiting and the wire; the output is wrong. Tells "the round trip is slow" apart from "the operations are". |
