...
| `DFLASH_CLUSTER_FAST_REDUCE` | ⚠️ **not usable yet.** Lean decode-path reduction: each rank writes its partial straight into the peers' pre-registered buffers instead of calling a collective. Connects and reduces correctly (1800 reductions, no timeouts) but runs at 34 ms per reduction against RCCL's 117 us; see server/docs/QWEN4EXP.md. Needs `--ulimit memlock=-1` on the container. |
| `DFLASH_CLUSTER_FAST_REDUCE_ELEMS` / `_SLOTS` | Size of the lean fabric: floats per reduction (default 262144, enough for a prefill chunk) and in-flight ring slots (default 128; a step has ~97). Anything larger than `_ELEMS` falls back to RCCL. |
